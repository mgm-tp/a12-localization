...
Usually, a fallback from a specific locale to a partial locale makes sense, e.g. from "en_US" to just "en".
...
[source,tsx,options="nowrap"]
----
export function SampleComponent(props: { /* ... **/ }): JSX.Element {
	const localizer = React.useContext(LocalizerContext).localizer;
	const myLocalizedText = localizer({
		key: "my.localizable.key",
		args: {},
		defaults: { en: "My default text in English" }
	})

	return <div>{myLocalizedText}</div>;
}
----
...
[source,tsx,options="nowrap"]
----
export function ComponentWithCustomLocalization(props: { locale: Locale }): JSX.Element {
	return (
...
----
const customDataFormatsProvider = (locale: Locale): DataFormats => {
	return { /** insert some custom implementation */};
}

const customLocalizerFactory = (locale: Locale, dataFormats: DataFormats): Localizer => {
	/** insert some custom implementation, sample below */

	// "debugging" localizer that returns the key of the first localizable and the locale itself
	// all text elements of the application UI will then show their primary localizable key
	return (localizable: Localizable) => {
		return `${localizable.key} (${Locale.toString(locale)})`;
	};
}

const customConversion: ValueConversion = {
	parseValue: (value, inputFormat) => {
		/** insert some custom implementation */
		return {
			value: undefined
		}
	},
...
export function Application(props: { locale: Locale }): JSX.Element {
	const localizer = React.useMemo<Localizer>(
		() => customLocalizerFactory(props.locale, customDataFormatsProvider(props.locale)),
		[props.locale]
	);
	...
	return (
		<LocalizerContext.Provider
			value={{
				locale: props.locale,
				dataFormats: customDataFormatsProvider(props.locale),
...
* `Locale.fromString()`: Creates a `Locale` object from its string representation, e.g. "en_US".
...
export const RESOURCE_KEYS: {
	dialogs: {
		confirmation: {
			...
			/** Key of the confirmation dialog message */
			message: "",

			/** Key of the confirmation dialog confirm button label */
			confirm: "",

			/** Key of the confirmation dialog cancel button label */
			cancel: ""
		},
		genericError: {
			/** Key of the generic error dialog title */
			title: "",

			/** Key of the generic error dialog message */
			message: "",

			/** Key of the generic error dialog close button label */
			close: ""
		}
	}
}
...
const en: typeof RESOURCE_KEYS = {
	dialogs: {
		confirmation: {
			...
			message: "Do you want to submit your changes?",
			confirm: "Submit",
			cancel: "Discard"
		},
		genericError: {
			title: "An unexpected error occurred",
			message: "The following error occurred: $errorMessage$",
			close: "Close"
		}
	}
};

const de: typeof RESOURCE_KEYS = { /* German translations */ };
...
[NOTE]
====
Since the localization tree node names are joined together to form a localizable key, they may not contain the `.` or `\` characters as explained in the section above.
If they are, however, supposed to contain it to match certain naming conventions, they must use the escaped formats: `\p` or `\\`.
====

[source,typescript,options="nowrap"]
----
// exemplary section of a render code for a generic error dialog

const titleLocalizable = localizableFromLocalizationTreeMap(
							RESOURCE_KEYS.dialogs.genericError.title,
							DEFAULT_TRANSLATIONS
						);
const messageLocalizable = localizableFromLocalizationTreeMap(
								RESOURCE_KEYS.dialogs.genericError.message,
								DEFAULT_TRANSLATIONS,
								// this specifies the value for the $errorMessage$ placeholder
								{ errorMessage: { type: "plain", value: genericError.message } }
							);

const title = localizer(titleLocalizable);
...
[source,tsx,title="Overriding the A12-default translations & fallback locale(s)"]
----
include::./typescript/usage.tsx[indent=0,tags=imports;ex1]
----

[source,tsx,title="Implementing a model-dependent localizer"]
----
include::./typescript/usage.tsx[indent=0,tags=imports;ex2]
----

[source,tsx,title="Customizing everything"]
----
include::./typescript/usage.tsx[indent=0,tags=imports;ex3]
----

= API Documentation

The API documentation for the `@com.mgmtp.a12.utils/utils-localization` can be found by following this link:./assets/localization-typedoc/index.html[link,role=external,window=_blank].

The API documentation for the `@com.mgmtp.a12.utils/utils-localization-react` can be found by following this link:./assets/localization-react-typedoc/index.html[link,role=external,window=_blank].
